    ...
    with open(csv_path, 'w', newline='') as out:
        ...
        for state in state_names:
            for index, state_zipcode_data in enumerate(shared_state_rep_map[state]):
                print("(persist_info_to_csv) state:",
                      state, state_zipcode_data)
...
def gather_zip_codes(start_end_tuple):
    ...
    for state in sns:
        ...
        # break
        print(
            f"Process {os.getpid()}: The total # of zip codes in {state}: {len(shared_state_zipcode_data_map[state])}")
    print("DONE")
    browser.close()
...
with multiprocessing.Manager() as manager:
    ...
    for cpu in range(NUM_CPUS):
        ...
        print("gather_zip_codes ==> offset, states_to_process:",
        ...
        processes.append(
            multiprocessing.Process(target=gather_zip_codes, args=(
                (offset, states_to_process, shared_state_rep_map),))
        )
        offset = states_to_process
    # os.kill(os.getpid(), 9)
    start_time = time()
    ...
    for cpu in range(NUM_CPUS):
        states_to_process = (int(total_states/8) + 1) + offset
        print("map_zc_to_rep ==> offset, states_to_process:",
              offset, states_to_process)
        processes.append(
